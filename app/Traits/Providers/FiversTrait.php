...
trait FiversTrait
{
     ...
     * Validate URL string
     */
    private static function validApiEndpoint(string $url): bool
    {
        return !empty($url) && filter_var($url, FILTER_VALIDATE_URL) !== false;
    }

    /**
     * Safe HTTP POST wrapper that validates URL and catches exceptions
     * Returns the response on success, or false on failure
     *
     * @param string $url
     * @param array $data
